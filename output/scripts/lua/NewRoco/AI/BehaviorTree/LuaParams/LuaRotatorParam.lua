...
local LuaRotatorParam = Base:Extend("LuaRotatorParam")

function LuaRotatorParam:Ctor(enableMFBT)
  Base.Ctor(self, enableMFBT)
  self.type = LuaParamType.Rotator
end

function LuaRotatorParam:GetValue(AIController)
  ...
  elseif self.isMFBTEnable then
    return AIController:GetMfbbQuat(self.key):ToRotator()
  else
    return AIController.Blackboard:GetValueAsRotator(self.key)
  end
end

function LuaRotatorParam:SetValue(AIController, Value)
  if not self.useBlackboardKey then
    local NpcName = "nil"
  ...
  if self.isMFBTEnable then
    if AIController.LocalGlobalConfig.MFBTUpdateBlackboardValueToCpp then
      AIController:SetMfbbQuat(self.key, Value:ToQuat())
    end
  elseif not AIController.LocalGlobalConfig.BTreeUseLuaBlackboard or AIController.LocalGlobalConfig.BTreeDebugCppBlackboard then
    AIController.Blackboard:SetValueAsRotator(self.key, Value)
  end
end

function LuaRotatorParam:GetType()
  return LuaParamType.Rotator
end

return LuaRotatorParam
