...
local LuaEnumParam = Base:Extend("LuaEnumParam")

function LuaEnumParam:Ctor(enableMFBT)
  Base.Ctor(self, enableMFBT)
  self.type = LuaParamType.Enum
end

function LuaEnumParam:GetValue(AIController)
...
function LuaEnumParam:SetValue(AIController, Value)
  if not self.useBlackboardKey then
    local NpcName = "nil"
...
function LuaEnumParam:GetType()
  return LuaParamType.Enum
end
