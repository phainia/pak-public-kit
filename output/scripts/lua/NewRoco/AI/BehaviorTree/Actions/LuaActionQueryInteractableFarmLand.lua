...
function LuaActionQueryInteractableFarmLand:OnStart(AIController, ...)
  ...
  local npc = owner.Npc
  local OwnerPlayerId = npc:GetOwnerId()
  local player = _G.NRCModuleManager:DoCmd(_G.PlayerModuleCmd.GET_LOCAL_PLAYER)
  if player:GetServerId() ~= OwnerPlayerId then
  ...
  if pos and npcDict then
    resultLandInfo = FarmUtils.GetNearestLandAvailable(function(i, l)
      ...
      if plant_npc then
        ...
        return isUnitTypeWater and dist < PLANT_PET_WATER_DISTANCE and FarmUtils.IsLandWateringAvailable(i, l) or isUnitTypeGrass and dist < PLANT_PET_MANURE_DISTANCE and FarmUtils.IsLandFertilizingAvailable(i, l) or dist < PLANT_PET_PICK_DISTANCE and FarmUtils.IsLandHarvestingAvailable(i, l)
      else
        return false
      end
    end, pos)
