...
local UMG_MagicStampTab2_C = _G.NRCViewBase:Extend("UMG_MagicStampTab2_C")

function UMG_MagicStampTab2_C:OnConstruct()
  self:SetIsLight(false)
end

function UMG_MagicStampTab2_C:OnActive()
end

function UMG_MagicStampTab2_C:OnDeactive()
end

function UMG_MagicStampTab2_C:OnAddEventListener()
end

function UMG_MagicStampTab2_C:OnTouchEnded(MyGeometry, InTouchEvent)
  local SelectMagicStampIndex = _G.NRCModuleManager:DoCmd(TaskModuleCmd.GetSelectMagicStampIndex)
  if SelectMagicStampIndex ~= TaskEnum.MagicStampTabType.order then
    self:DispatchEvent(TaskModuleEvent.ChangeMagicStamp, TaskEnum.MagicStampTabType.order)
...
function UMG_MagicStampTab2_C:SetIsLight(Light)
  if Light then
...
function UMG_MagicStampTab2_C:RemoveSelected(_CurItemType)
  if _CurItemType == TaskEnum.MagicStampTabType.order then
