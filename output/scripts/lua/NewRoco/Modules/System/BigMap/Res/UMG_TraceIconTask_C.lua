local UMG_TraceIconTask_C = _G.NRCPanelBase:Extend("UMG_TraceIconTask_C")
local BigMapModuleEvent = reload("NewRoco.Modules.System.BigMap.BigMapModuleEvent")

function UMG_TraceIconTask_C:OnConstruct()
  self.NRCButton_ClickRange.OnClicked:Add(self, self.OnClickRange)
end

function UMG_TraceIconTask_C:OnDestruct()
  self.uiData = nil
  self.NRCButton_ClickRange.OnClicked:Remove(self, self.OnClickRange)
end

function UMG_TraceIconTask_C:OnClickRange()
  Log.Debug("UMG_TraceIconTask_C:OnClickRange")
  local BigMapModule = _G.NRCModuleManager:GetModule("BigMapModule")
  BigMapModule:DispatchEvent(BigMapModuleEvent.ClickTraceIconEvent, self)
end

function UMG_TraceIconTask_C:SetData(_data)
  self.uiData = _data
  self:UpdatePanel()
end

function UMG_TraceIconTask_C:IsUsable()
  return self.uiData ~= nil
end

function UMG_TraceIconTask_C:GetImagePosition()
  return self.uiData.imagePosX or 0, self.uiData.imagePosY or 0
end

function UMG_TraceIconTask_C:IsTaskTrace()
  return true
end

function UMG_TraceIconTask_C:UpdateImagePosition(_posX, _posY)
  local uiData = self.uiData
  uiData.imagePosX = _posX or 0
  uiData.imagePosY = _posY or 0
end

function UMG_TraceIconTask_C:SetVisible(_isVisible)
  if self.isVisible == _isVisible then
    return
  end
  self.isVisible = _isVisible
  if self.isVisible then
    self:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
  else
    self:SetVisibility(UE4.ESlateVisibility.Collapsed)
  end
end

function UMG_TraceIconTask_C:UpdatePanel()
  if not self.uiData then
    self:SetVisible(false)
  end
end

function UMG_TraceIconTask_C:SetArrowDir(_angle)
  local dirMat = self.dirIcon1:GetDynamicMaterial()
  if dirMat then
    dirMat:SetScalarParameterValue("Angle", 90 - _angle)
  end
  self.dirIcon1:SetRenderTransformAngle(90 - _angle)
end

return UMG_TraceIconTask_C
