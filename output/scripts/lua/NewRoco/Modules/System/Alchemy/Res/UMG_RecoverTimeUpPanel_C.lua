...
local UMG_RecoverTimeUpPanel_C = _G.NRCPanelBase:Extend("UMG_RecoverTimeUpPanel_C")

function UMG_RecoverTimeUpPanel_C:OnActive(data)
  if _G.GlobalConfig.DebugOpenUI then
    self:OnAddEventListener()
    return
  end
  local title = _G.DataConfigManager:GetLocalizationConf("alchemy_bottle_times_title")
  self.Title:SetText(title and title.msg or "\232\175\183\233\133\141\231\189\174alchemy_bottle_times_title")
  local insufficientText = _G.DataConfigManager:GetLocalizationConf("alchemy_bottle_times_item_short")
  ...
  local MaxLevelHint = _G.DataConfigManager:GetLocalizationConf("alchemy_bottle_times_is_max")
  self.isMaxLevelHint = MaxLevelHint and MaxLevelHint.msg or "\232\175\183\233\133\141\231\189\174alchemy_bottle_times_is_max"
  local LevelRequiredNotMeet = _G.DataConfigManager:GetLocalizationConf("alchemy_bottle_times_grade_short")
  ...
  self.LevelUpText = LevelUpButtonText and LevelUpButtonText.msg or "\233\133\141\231\189\174\228\184\162\228\186\134\232\175\183\230\137\190\231\173\150\229\136\146"
  self.UMG_CoinButton:SetClickAble(true)
  self.UMG_CoinButton:SetBtnText(self.LevelUpText)
...
function UMG_RecoverTimeUpPanel_C:OnAnimationFinished(Animation)
  ...
  elseif Animation == self.close and self.shouldClose then
    _G.NRCEventCenter:DispatchEvent(_G.AlchemyModuleEvent.RecoverTimeUpPanelClosed)
    self:DoClose()
  end
end

function UMG_RecoverTimeUpPanel_C:OnDeactive()
  self:OnRemoveEventListener()
end

function UMG_RecoverTimeUpPanel_C:OnAddEventListener()
...
function UMG_RecoverTimeUpPanel_C:OnPcClose()
  if self.ReturnBtn:IsVisible() then
    self:OnClose()
  end
end
...
function UMG_RecoverTimeUpPanel_C:UpdatePanel()
  self.data = _G.NRCModuleManager:DoCmd(_G.AlchemyModuleCmd.GetBottleTimeData)
  ...
  if not self.ItemEnough then
    ...
    self.UMG_CoinButton2.Title_1:SetText(self.LevelUpText)
  ...
  if 0 == #self.RestoreList then
    ...
    for i = math.max(self.data.origin_value + 1, 1), self.data.max_value do
      table.insert(self.RestoreList, {filled = false})
      break
    end
  end
...
function UMG_RecoverTimeUpPanel_C:UpdateCostIcon(exchangeId, item_num)
...
function UMG_RecoverTimeUpPanel_C:OnClose()
...
function UMG_RecoverTimeUpPanel_C:OnLevelUp()
  if not UIUtils.IsClickable(self) then
    return
  end
  if self:IsPlayingAnimation() then
  ...
  _G.NRCAudioManager:PlaySound2DAuto(41401001, "UMG_RecoverTimeUpPanel_C:OnLevelUp")
  if self.ClickEnable then
    self:SetVisibility(UE4.ESlateVisibility.HitTestInvisible)
    _G.NRCModuleManager:DoCmd(_G.AlchemyModuleCmd.DisableClick)
    _G.NRCModuleManager:DoCmd(_G.AlchemyModuleCmd.RequestForUpgrade, _G.Enum.VisualItem.VI_BOTTLE_TIMES, self.data.upgradeId, self.data.exchangeId, self.data.origin_value, self.data.target_value)
  else
...
function UMG_RecoverTimeUpPanel_C:ShowClose()
  self:SetVisibility(UE4.ESlateVisibility.HitTestInvisible)
...
function UMG_RecoverTimeUpPanel_C:ShowOpen()
  self:StopAllAnimations()
  self:PlayAnimation(self.open)
  self:UpdatePanel()
end

return UMG_RecoverTimeUpPanel_C
