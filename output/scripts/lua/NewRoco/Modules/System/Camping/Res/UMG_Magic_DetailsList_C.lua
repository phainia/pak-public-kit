...
local UMG_Magic_DetailsList_C = Base:Extend("UMG_Magic_DetailsList_C")

function UMG_Magic_DetailsList_C:OnConstruct()
end

function UMG_Magic_DetailsList_C:OnDestruct()
end

function UMG_Magic_DetailsList_C:OnItemUpdate(_data, datalist, _index)
  self.index = _index
  self.uiData = _data
  self:SetData()
end

function UMG_Magic_DetailsList_C:OnDeactive()
end

function UMG_Magic_DetailsList_C:SetData()
  local PropIcon, Quality = self:GetPet3DIconPath(self.uiData.cfg.id)
  self.ItemIcon:SetPath(PropIcon)
  self:SetQuality(Quality)
  local showOrHideMask = self.uiData.isLvOK == true and UE4.ESlateVisibility.Hidden or UE4.ESlateVisibility.Visible
  self.The_Black_Cap:SetVisibility(showOrHideMask)
  self.Sigh:SetVisibility(showOrHideMask)
  self.Chain:SetVisibility(true == self.uiData.hasLink and UE4.ESlateVisibility.Hidden or UE4.ESlateVisibility.Visible)
end

function UMG_Magic_DetailsList_C:GetPet3DIconPath(id)
  local itemConf
  local propIconPath = ""
  local quality = 0
...
function UMG_Magic_DetailsList_C:SetQuality(quality)
...
function UMG_Magic_DetailsList_C:OnItemSelected(_bSelected)
  if _bSelected then
    ...
    local param = {
      ...
      isSketch = false,
      insufficientLv = not self.uiData.isLvOK
    }
    _G.NRCModeManager:DoCmd(TipsModuleCmd.Tips_OpenMagicDetailTips, param)
  end
end

function UMG_Magic_DetailsList_C:PetHasAcquired(petbaseId)
  ...
  for i = 1, #handBookInfo do
    if not handBookInfo[i].record or 0 == #handBookInfo[i].record then
      ...
      if record.pet_base_id == petbaseId and record.study_lv then
        return true
      end
    end
  end
...
return UMG_Magic_DetailsList_C
