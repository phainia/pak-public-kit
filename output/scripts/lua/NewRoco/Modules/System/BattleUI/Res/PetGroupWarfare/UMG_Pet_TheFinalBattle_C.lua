...
local UMG_Pet_TheFinalBattle_C = _G.NRCPanelBase:Extend("UMG_Pet_TheFinalBattle_C")

function UMG_Pet_TheFinalBattle_C:OnConstruct()
  self.TeammateHPList = {
...
function UMG_Pet_TheFinalBattle_C:OnDestruct()
...
function UMG_Pet_TheFinalBattle_C:OnAddEventListener()
  ...
  _G.BattleEventCenter:Bind(self, BattleEvent.PLAYER_CHOOSE_SKILL, BattleEvent.PLAYER_PERFORM_OVER, BattleEvent.TEAM_BATTLE_CATCH, BattleEvent.BATTLE_PET_DIE, BattleEvent.Replay_RefreshRoundIdx, BattleEvent.HIDE_TEAM_HP, BattleEvent.PET_SPAWNED, BattleEvent.HIDE_TEAMBATTLE_HP, BattleEvent.SHOW_TEAMBATTLE_HP, BattleEvent.TEAM_SWAP_SELECT_START)
  self:AddButtonListener(self.HeadIconBtn, self.OnOpenPetTips)
end

function UMG_Pet_TheFinalBattle_C:OnRemoveEventListener()
  _G.BattleEventCenter:UnBind(self)
  self:RemoveButtonListener(self.HeadIconBtn, self.OnOpenPetTips)
end

function UMG_Pet_TheFinalBattle_C:OnActive()
...
function UMG_Pet_TheFinalBattle_C:OnBattleEvent(eventName, ...)
...
function UMG_Pet_TheFinalBattle_C:HideHp()
  self:SetVisibility(UE4.ESlateVisibility.Collapsed)
...
function UMG_Pet_TheFinalBattle_C:PlayOpen()
  self:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
  self:PlayAnimation(self.open)
end

function UMG_Pet_TheFinalBattle_C:ShowPanel()
  self:StopAllAnimations()
  self:PlayOpen()
end

function UMG_Pet_TheFinalBattle_C:HidePanel()
  self:StopAllAnimations()
  self:PlayAnimation(self.Close)
end

function UMG_Pet_TheFinalBattle_C:HideTeamBattleHp()
...
function UMG_Pet_TheFinalBattle_C:ShowTeamBattleHp()
  for i, TeammateHP in ipairs(self.TeammateHPList) do
    if 1 ~= i then
...
function UMG_Pet_TheFinalBattle_C:OnSelfSelectPet()
  for i, TeammateHP in ipairs(self.TeammateHPList) do
    TeammateHP:SetVisibility(UE4.ESlateVisibility.Collapsed)
  end
...
function UMG_Pet_TheFinalBattle_C:TryInitData()
...
function UMG_Pet_TheFinalBattle_C:ShowTipTime()
  local params = {}
...
function UMG_Pet_TheFinalBattle_C:UpdateSkillList()
  _G.NRCModeManager:DoCmd(BattleUIModuleCmd.CloseSkillTips)
end

function UMG_Pet_TheFinalBattle_C:PetDleUpdatePanel(battlePet)
  if self.OwnPet and self.OwnPet.guid == battlePet.guid then
    self:UpdateRound()
  end
end

function UMG_Pet_TheFinalBattle_C:UpdateRound(Round)
  if self.OwnPet and self.OwnPet:IsDead() and self.OwnPet.player:GetSummonNumber() > 0 then
    ...
    Log.Warning("UMG_Pet_TheFinalBattle_C:UpdateRound ", RoundInfo, self.OwnPet.card.petInfo.battle_inside_pet_info.revive_round, self.OwnPet.card.petInfo.battle_inside_pet_info.revive_rounds, Round)
...
function UMG_Pet_TheFinalBattle_C:InitView(battlePet)
...
function UMG_Pet_TheFinalBattle_C:UpdateStarChain()
...
function UMG_Pet_TheFinalBattle_C:SetTeamPetInfo(battlePet)
...
function UMG_Pet_TheFinalBattle_C:UpdateSkillInoList()
...
function UMG_Pet_TheFinalBattle_C:SelectRecoveryItem(selectItemIndex)
  self.SummaryRecall:SelectItemByIndex(selectItemIndex)
end

function UMG_Pet_TheFinalBattle_C:FindPetPos(pet_id)
  for i, TeammateHP in ipairs(self.TeammateHPList) do
...
function UMG_Pet_TheFinalBattle_C:FindOwnPet(pet_id)
...
function UMG_Pet_TheFinalBattle_C:SetEnemyPetInfo(battlePet)
  if battlePet then
...
function UMG_Pet_TheFinalBattle_C:updatePetGender(_gender)
...
function UMG_Pet_TheFinalBattle_C:SetTypes(card)
...
function UMG_Pet_TheFinalBattle_C:SetBloodPulseIcon(_petData)
...
function UMG_Pet_TheFinalBattle_C:IsPCMode()
  return UE.UGameplayStatics.GetGameInstance(self):IsPCMode()
end

function UMG_Pet_TheFinalBattle_C:PCModeScreenSetting()
...
function UMG_Pet_TheFinalBattle_C:OnBtnRechristen_1Click()
...
function UMG_Pet_TheFinalBattle_C:OnBloodBtn()
...
function UMG_Pet_TheFinalBattle_C:OnDeactive()
end

function UMG_Pet_TheFinalBattle_C:UpdateEnemySkillList(_curRound)
...
function UMG_Pet_TheFinalBattle_C:RefreshCatchConsumeInfo(itemType)
  if self.EnemyPet and self.EnemyPet.battlePetComponents then
    self.EnemyPet.battlePetComponents:RefreshCatchConsumeInfo(itemType)
  end
end

function UMG_Pet_TheFinalBattle_C:PlayRemoveAnim(_curRound)
...
function UMG_Pet_TheFinalBattle_C:UpdateList()
...
function UMG_Pet_TheFinalBattle_C:ShowEnemySkillListByChooseSkill(_PetSkillList, _curRound)
...
function UMG_Pet_TheFinalBattle_C:IsChangePetRound(_curRound)
...
function UMG_Pet_TheFinalBattle_C:SetRemoveFlags(_curRound)
  for i, Skill in ipairs(self.EnemySkillList) do
...
function UMG_Pet_TheFinalBattle_C:RefreshChildClickAnim()
  local targetPos = 0
...
function UMG_Pet_TheFinalBattle_C:PlayChildClickAnim(Index)
...
function UMG_Pet_TheFinalBattle_C:CheckIsWaitingOtherOrDie()
  local isWaitingOther = _G.BattleManager:CheckActiveState(BattleEnum.StateNames.WaitingOther)
  if isWaitingOther then
    return true
  end
...
function UMG_Pet_TheFinalBattle_C:RemoveBeforeRound(curRound)
...
function UMG_Pet_TheFinalBattle_C:ShowFirstFourSkills(curRound, _IsBoost)
  local ShowSkillList = {}
  local CurRound = 0
  if _IsBoost then
    CurRound = curRound + 4
...
function UMG_Pet_TheFinalBattle_C:AddNewRound(_PetSkillList, _curRound)
  local CurRound = _curRound
  if _PetSkillList.skills then
    for i, skill_id in ipairs(_PetSkillList.skills) do
      ...
      if not IsHasSkill or i == #_PetSkillList.skills then
...
function UMG_Pet_TheFinalBattle_C:SortRound()
...
function UMG_Pet_TheFinalBattle_C:OnAnimationFinished(Animation)
...
function UMG_Pet_TheFinalBattle_C:OnOpenPetTips()
...
return UMG_Pet_TheFinalBattle_C
