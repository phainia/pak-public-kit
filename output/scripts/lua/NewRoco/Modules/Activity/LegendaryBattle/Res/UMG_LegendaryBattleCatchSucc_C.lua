...
local UMG_LegendaryBattleCatchSucc_C = _G.NRCPanelBase:Extend("UMG_LegendaryBattleCatchSucc_C")

function UMG_LegendaryBattleCatchSucc_C:OnConstruct()
  ...
  self.UMG_PetRate:SetTipDesirePanelLayer(Enum.UILayerType.UI_LAYER_POPUP)
...
function UMG_LegendaryBattleCatchSucc_C:OnActive(_Param, PetLevel)
  ...
  self.PetLevel = self.module.curChooseStarNum
  self:SetPanelInfo()
end

function UMG_LegendaryBattleCatchSucc_C:OnDestruct()
  _G.UpdateManager:UnRegister(self)
end

function UMG_LegendaryBattleCatchSucc_C:OnPcClose()
  if self.HotArea:IsVisible() and not self:IsPlayingAnimation() then
    self:OnCloseBtnClick()
  end
end

function UMG_LegendaryBattleCatchSucc_C:OnDeactive()
  _G.NRCAudioManager:BatchSetState("UI_Music;None")
end

function UMG_LegendaryBattleCatchSucc_C:OnAddEventListener()
...
function UMG_LegendaryBattleCatchSucc_C:OnBtnRechristen_1Click()
...
function UMG_LegendaryBattleCatchSucc_C:OnBloodBtn()
...
function UMG_LegendaryBattleCatchSucc_C:SetTalentRank(petData)
  self.UMG_PetRate:SetText(petData)
end

function UMG_LegendaryBattleCatchSucc_C:OnCloseBtnClick()
  _G.BattleEventCenter:Dispatch(BattleEvent.CLICKED_Result_Close)
  self:PlayAnimation(self.Out)
end

function UMG_LegendaryBattleCatchSucc_C:SetPanelInfo()
...
function UMG_LegendaryBattleCatchSucc_C:SetPetInfo(reward)
  ...
  if petBaseCfg then
    ...
    self.Text_Name:SetText(petBaseCfg.name)
    if petBaseCfg then
      self:updatePetTypeIcon(petBaseCfg.unit_type)
    end
    local notice = string.format(LuaText.legendary_battle_tips_2, petBaseCfg.name)
    self.NoticeText:SetText(notice)
  end
  local Levels = {}
  ...
  self.StarList:InitGridView(Levels)
  self:SetBloodlineBattle()
...
function UMG_LegendaryBattleCatchSucc_C:updatePetGender(_gender)
...
function UMG_LegendaryBattleCatchSucc_C:UpdateMDT_SHINING()
  self.CanvasPanel_117:SetVisibility(UE4.ESlateVisibility.Collapsed)
  self.CanvasPanel_48:SetVisibility(UE4.ESlateVisibility.Collapsed)
  self.DifferentColorsDazzling:SetVisibility(UE4.ESlateVisibility.Collapsed)
  self.DazzlingColors:SetVisibility(UE4.ESlateVisibility.Collapsed)
  self.DazzlingColors:InitUI(self.PetData)
end

function UMG_LegendaryBattleCatchSucc_C:SetBloodlineBattle()
  if self.Attr then
    local typeList = {}
...
function UMG_LegendaryBattleCatchSucc_C:SetNature()
  local natureConfNormal = _G.DataConfigManager:GetNatureConf(self.PetData.nature)
  if natureConfNormal.name then
    self.Department_3:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    self.Text_3:SetText(natureConfNormal.name or "")
  else
    self.Department_3:SetVisibility(UE4.ESlateVisibility.Collapsed)
  end
end

function UMG_LegendaryBattleCatchSucc_C:updatePetTypeIcon(_dicTypes)
  local typeList = {}
...
function UMG_LegendaryBattleCatchSucc_C:SetPetAttri()
...
function UMG_LegendaryBattleCatchSucc_C:GetChangeAttrReqEnum(attribute)
...
function UMG_LegendaryBattleCatchSucc_C:OnAnimationFinished(Anim)
  if Anim == self.Out then
    self:DoClose()
  end
end

function UMG_LegendaryBattleCatchSucc_C:OnBtnCollectClick()
  if self.PetData then
    _G.NRCModeManager:DoCmd(PetUIModuleCmd.OpenPetCollectPanel, self.PetData.gid, self.partner_mark or 0)
  end
end

function UMG_LegendaryBattleCatchSucc_C:UpdateCollect(partner_mark)
  self.partner_mark = partner_mark
  self.UMG_CollectBtn:UpdateInfo(partner_mark)
end

return UMG_LegendaryBattleCatchSucc_C
