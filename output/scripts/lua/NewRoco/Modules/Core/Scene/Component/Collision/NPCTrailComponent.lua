...
function NPCTrailComponent:Ctor()
  Base.Ctor(self)
...
function NPCTrailComponent:OnDistanceOptimize(distance, viewDotValue, bulkyVisible, distanceRatio)
  ...
  local config = self.owner and self.owner.config
  local npc_trampling_lawn_comp = config and config.npc_trampling_lawn_comp
  if not npc_trampling_lawn_comp or 0 == npc_trampling_lawn_comp then
    return
  end
  ...
  if distance < TriggerRange and not self.isTrigger then
    ...
    if npc_trampling_lawn_comp == _G.Enum.TramplingLawnComp.TLC_DYNAMIC then
