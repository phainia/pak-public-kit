...
local OffDashAbility = Base:Extend("OffDashAbility")

function OffDashAbility:Init(abilityConf)
  Base.Init(self, abilityConf)
  self._buffName = "DashBuff"
end

function OffDashAbility:Start(onFinished)
  Base.Start(self, onFinished)
  local player = self.caster
  player.buffComponent:RemoveBuff(self._buffName)
end

function OffDashAbility:Recover(owner)
  owner.buffComponent:RemoveBuff(self._buffName)
end

return OffDashAbility
