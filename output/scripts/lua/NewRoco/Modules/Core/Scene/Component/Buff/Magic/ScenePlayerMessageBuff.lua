...
local ScenePlayerMessageBuff = Base:Extend("ScenePlayerMessageBuff")
local TopKFinderNum = 8
local TopKDistance = 800

function ScenePlayerMessageBuff:OnBegin(owner, MagicInfo)
...
function ScenePlayerMessageBuff:CreateLocalNPC()
...
function ScenePlayerMessageBuff:SetTrajectory(ShouldShow)
  ShowTrajectory = ShouldShow
end

function ScenePlayerMessageBuff:OnTick()
  if self.owner == nil or not self.owner.isLocal then
...
function ScenePlayerMessageBuff:OnNpcLoaded(viewObj)
  if not viewObj then
    return
  end
  ...
  if npc:GetComponent(MessageMagicComponent) == nil then
    npc:EnsureComponent(MessageMagicComponent)
  end
...
function ScenePlayerMessageBuff:OnNpcDestroyed(npc)
  if npc then
...
function ScenePlayerMessageBuff:UpdateNpcTransform()
  local npc = self.magicInfo.npc
...
function ScenePlayerMessageBuff:CheckNpcValid()
...
function ScenePlayerMessageBuff:GetNpcValidType(npc)
  if nil == npc then
...
function ScenePlayerMessageBuff:ConvertValidType(type)
...
function ScenePlayerMessageBuff:ConstValidateTopK(npc, ignore)
  if not npc then
    return false
  end
...
function ScenePlayerMessageBuff:OnFinish()
  Base.OnFinish(self, true)
  _G.UpdateManager:UnRegister(self)
end

return ScenePlayerMessageBuff
