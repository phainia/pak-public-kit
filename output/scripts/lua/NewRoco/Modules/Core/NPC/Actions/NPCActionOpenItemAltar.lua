...
local NPCActionOpenItemAltar = Base:Extend("NPCActionOpenItemAltar")

function NPCActionOpenItemAltar:Ctor(Owner, Config, Info)
  Base.Ctor(self, Owner, Config, Info)
end

function NPCActionOpenItemAltar:Execute()
  Log.Debug("NPCActionOpenItemAltar:Execute")
  Base.Execute(self)
  local player = self:GetPlayer()
  player:PlayAnim("Think", 1, 0, 0.1, 0.1, 999, 0, "Locomotion")
  _G.NRCModuleManager:DoCmd(AltarModuleCmd.OpenItemAltarPanel, self)
end

function NPCActionOpenItemAltar:GiveFinish()
  local Player = self:GetPlayer()
  ...
  if not Skill then
    Log.Error("NPCActionOpenItemAltar:GiveFinish \230\137\190\228\184\141\229\136\176Skill")
    return
  end
...
function NPCActionOpenItemAltar:OnSetupBlackboard(Name, Skill)
...
function NPCActionOpenItemAltar:OnSkillCallBack(skillProxy, result)
...
function NPCActionOpenItemAltar:SkillFailed(success, data, param)
  Base.Finish(self, success, data, param)
end

function NPCActionOpenItemAltar:SubmitAction(success, data, param)
  self:Finish(true, nil, "1")
end

function NPCActionOpenItemAltar:Finish(success, data, param)
  local localPlayer = self:GetPlayer()
  localPlayer:StopAnim("Think", 0.1, "Locomotion")
  Base.Finish(self, success, data, param)
end

return NPCActionOpenItemAltar
