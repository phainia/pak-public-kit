...
local FsmDoCmdAsyncAction = Base:Extend("FsmDoCmdAsyncAction")
FsmUtils.MergeMembers(Base, FsmDoCmdAsyncAction, {
...
function FsmDoCmdAsyncAction:Ctor(name, properties)
  Base.Ctor(self, name, properties)
end

function FsmDoCmdAsyncAction:OnEnter()
  ...
  if string.IsNilOrEmpty(Cmd) then
    ...
    local Result = _G.NRCModeManager:DoCmdAsync({
      owner = self,
...
function FsmDoCmdAsyncAction:OnFinish()
  self.fsm:Resume()
end

function FsmDoCmdAsyncAction:OnExit()
end

return FsmDoCmdAsyncAction
