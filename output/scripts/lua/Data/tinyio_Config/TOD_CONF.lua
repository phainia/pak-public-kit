...
function dataTable:addCacheRecord(_key, r)
  local lua_record = {}
  lua_record.id = r.id
  lua_record.tod_enum = r.tod_enum
  ...
  lua_record.temp = r.temp
  TOD_CONF[_key] = lua_record
end

function dataTable:GetData(_key)
  local r = TOD_CONF[_key]
  if nil == r then
    local r = TinyData.GetRecord("TOD_CONF", _key)
    if nil ~= r then
      self:addCacheRecord(_key, r)
      return TOD_CONF[_key]
    else
      return nil
    end
  end
...
function dataTable:GetAllDatas()
  local confs = TinyData.GetTable("TOD_CONF", true)
  local count = TinyDataTable.GetRecordsCount(confs)
  if count == #TOD_CONF then
    return TOD_CONF
  end
...
function dataTable:GetDataCount()
  return TinyData.GetRecordsCount("TOD_CONF")
end
