...
function dataTable:addCacheRecord(_key, r)
  ...
  GLOBAL_CONFIG[_key] = lua_record
end

function dataTable:GetData(_key)
  local r = GLOBAL_CONFIG[_key]
  if nil == r then
    local r = TinyData.GetRecord("GLOBAL_CONFIG", _key)
    if nil ~= r then
...
function dataTable:GetAllDatas()
  local confs = TinyData.GetTable("GLOBAL_CONFIG", true)
  local count = TinyDataTable.GetRecordsCount(confs)
  if count == #GLOBAL_CONFIG then
...
function dataTable:GetDataCount()
  return TinyData.GetRecordsCount("GLOBAL_CONFIG")
end
