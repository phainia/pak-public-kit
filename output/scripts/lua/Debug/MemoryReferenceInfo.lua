...
local CollectObjectReferenceInMemory = function(strName, cObject, cDumpInfoContainer)
  if not cObject then
    return
  end
  strName = strName or ""
...
local CollectSingleObjectReferenceInMemory = function(strName, cObject, cDumpInfoContainer)
  if not cObject then
    return
  end
  strName = strName or ""
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults)
  ...
  if bOutputFile then
    ...
    Log.Debug("FilePath : " .. strFileName)
    finalPath = strFileName
...
local function OutputFilteredResult(strFilePath, strFilter, bIncludeFilter, bOutputFile)
  ...
  local cOutputEntry = print
  if bOutputFile then
    local strResFileName = strFilePath
...
local function DumpMemorySnapshotComparedFile(strSavePath, strExtraFileName, nMaxRescords, strResultFilePathBefore, strResultFilePathAfter)
  ...
  local outputFilePath = OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, nil, nil, cResultBefore, cResultAfter)
