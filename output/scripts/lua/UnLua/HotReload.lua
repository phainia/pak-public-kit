...
local function dump(tbl, max_indent)
  ...
  local handled = {}
  local traverse = function(tbl, indent)
    indent = indent or 0
  ...
  local function require(module_name, ...)
    ...
    if func then
      ...
      if nil == loaded_modules[module_name] then
        ...
        LuaClassExpand(new_module, module_name)
...
local function collect_module_upvalues(moudule)
  local collect_function_upvalues = function(func, upvalues)
...
local function update_global(value_map)
  ...
  local update_table
  local update_running_stack = function(co, level)
