...
function LoginObserver:OnLoginRetNotify(LoginRet)
  ...
  if not LoginFsm then
    ...
    if not NRCModuleManager:GetModule("LoginModule") then
      ...
      elseif LoginRet.retCode == UE.MSDKError.SUCCESS then
        ...
        NRCModuleManager:DoCmd(OnlineModuleCmd.SetUserAccountInfo, tostring(LoginRet.openID), tostring(LoginRet.token), tostring(LoginRet.channel), reg_channel_dis, cli_startup_channel)
  ...
  elseif LoginRet.retCode == UE.MSDKError.SUCCESS then
    ...
    NRCModuleManager:DoCmd(OnlineModuleCmd.SetUserAccountInfo, tostring(LoginRet.openID), tostring(LoginRet.token), tostring(LoginRet.channel), LoginRet.regChannelDis, cli_startup_channel)
