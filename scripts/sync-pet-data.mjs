...
const publicDataDir = path.resolve(process.argv[2] ?? path.join(rootDir, "output", "data"));
...
function buildMoveEntries(details) {
    const movesById = new Map();

    for (const detail of details) {
        for (const move of [
            ...normalizeArray(detail.move_pool),
            ...normalizeArray(detail.move_stones),
            ...normalizeArray(detail.legacy_moves).map((entry) => entry?.move),
        ]) {
            if (!Number.isFinite(move?.id) || movesById.has(move.id)) {
                continue;
            }
    ...
    return Array.from(movesById.values()).sort((left, right) => left.id - right.id);
}

function buildMagicItemEntries(itemEntries) {
    return itemEntries
        .filter((item) => {
            const category = item.category ?? "";
            const typeDesc = item.type_desc ?? "";
            return (
                category.includes("共鸣魔法") ||
                typeDesc.includes("共鸣魔法") ||
                (category.includes("魔法") && typeDesc.includes("魔法"))
            );
        })
        .map((item) => ({
            id: item.id,
